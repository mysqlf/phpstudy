...
<body>
    ...
    <!-- bdright -->
    <div id="bdright">
        <div class="site"><a href="<?php echo U('Auth/role');?>" class="active"><i class="icon-th-large"></i> 角色列表</a><a href="<?php echo U('Auth/add_role');?>"><i class="icon-user-plus"></i> 添加角色</a></div>
        <div class="content">
            <table class="table table-border table-bordered table-striped table-hover">
                <thead>
                    <tr>
                        <th width="200px">角色描述</th>
                        <th class="text-c" width="50px">状态</th>
                        <th>操作</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(($vo["id"]) != "1"): ?><tr>
                        <th><?php echo ($vo["title"]); ?></th>
                        <td class="text-c">
                            <?php if(($vo["status"]) == "1"): ?><a onclick="ajaxjump('<?php echo U('Auth/role_status',array('id'=>$vo['id'],'s'=>0));?>')" href="javascript:;"><img src="/jf/Public/admin//ico_green.png" title="点击禁用"></a>
                            <?php else: ?>
                            <a onclick="ajaxjump('<?php echo U('Auth/role_status',array('id'=>$vo['id'],'s'=>1));?>')" href="javascript:;"><img src="/jf/Public/admin//ico_red.png" title="点击启用"></a><?php endif; ?>
                        </td>
                        <td>
                            <a href="<?php echo U('Auth/mod_role',array('id'=>$vo['id']));?>" class="btn btn-success radius size-S">修改</a>　<a onclick="confirm('<?php echo U('Auth/del_role',array('id'=>$vo['id']));?>','确定要删除吗？')" class="btn btn-danger radius size-S">删除</a>
                        </td>

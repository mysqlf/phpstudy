                    ...
                    <table class="table table-border table-bordered table-hover radius">
                        <?php if(is_array($fdata)): $i = 0; $__LIST__ = $fdata;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; switch($vo["type"]): case "1": ?><tr>
                                ...
                                <?php case "3": ?><tr>
                                    <th width="80px"><?php echo ($vo["title"]); ?></th>
                                    <td>
                                        <textarea <?php if(empty($vo["att"])): ?>style="height:90px;"<?php else: ?>style="height:<?php echo ($vo["att"]); ?>px;"<?php endif; ?> name="<?php echo ($vo["name"]); ?>" class="textarea radius le4" <?php if(empty($vo["regular"])): ?>datatype="*"<?php else: ?>datatype="<?php echo ($vo["regular"]); ?>"<?php endif; ?> <?php if(($vo["is_must"]) == "1"): ?>nullmsg="<?php echo ($vo["title"]); ?>必须填写"<?php else: ?>ignore="ignore"<?php endif; ?> errormsg="<?php echo ($vo["note"]); ?>"><?php echo ($vo["f_data"]); ?></textarea><span class="notic"> <?php echo ($vo["note"]); ?></span>
                                    </td>
                                </tr><?php break;?>
                                <?php case "4": ?><tr>
                                    <th width="80px"><?php echo ($vo["title"]); ?></th>
                                    <td>
                                        <?php echo form_select($vo['name'],$vo['f_data'],$vo['att'],$vo['title'],$vo['is_must']);?><span class="notic"> <?php echo ($vo["note"]); ?></span>
                                    </td>
                                </tr><?php break;?>
                                <?php case "5": ?><tr>
                                    <th width="80px"><?php echo ($vo["title"]); ?></th>
                                    <td>
                                        <?php echo form_ueditor($vo['name'],$vo['f_data'],$vo['att']);?>
                                    </td>
                                </tr><?php break;?>
                                <?php case "6": ?><tr>
                                    <th width="80px"><?php echo ($vo["title"]); ?></th>
                                    <td>
                                        <?php echo form_upload($vo['name'],$vo['att'],$vo['f_data']);?><span class="notic"> <?php echo ($vo["note"]); ?></span>
                                    </td>
                                </tr><?php break;?>
                                <?php case "7": ?><tr>
                                    <th width="80px"><?php echo ($vo["title"]); ?></th>
                                    <td>
                                        <?php echo form_uploads($vo['name'],$vo['att'],$vo['f_data']);?><span class="notic"> <?php echo ($vo["note"]); ?></span>
                                    </td>
                                </tr><?php break;?>
                                <?php case "8": ?><tr>
                                    <th width="80px"><?php echo ($vo["title"]); ?></th>
                                    <td><?php echo form_category($vo['name'],$vo['att'],$vo['f_data']);?><span class="notic"> <?php echo ($vo["note"]); ?></span></td>
                                </tr><?php break;?>
                                <?php case "9": ?><tr>
                                    <th width="80px"><?php echo ($vo["title"]); ?></th>
                                    <td>
                                        <?php echo form_date($vo['name'],$vo['title'],$vo['is_must'],$vo['f_data']);?><span class="notic"> <?php echo ($vo["note"]); ?></span>
                                    </td>
                                </tr><?php break;?>
                                <?php case "10": ?><tr>
                                    <th width="80px"><?php echo ($vo["title"]); ?></th>
                                    <td>
                                        <?php echo form_radio($vo['name'],$vo['f_data'],$vo['att'],$vo['title'],$vo['is_must']);?><span class="notic"> <?php echo ($vo["note"]); ?></span>
                                    </td>
                                </tr><?php break;?>
                                <?php case "11": ?><tr>
                                    <th width="80px"><?php echo ($vo["title"]); ?></th>
                                    <td>
                                        <?php echo form_ueditor2($vo['name'],$vo['f_data'],$vo['att']);?>
                                    </td>
                                </tr><?php break; endswitch; endforeach; endif; else: echo "" ;endif; ?>
                    </table>
                </div>
            </div>
            <div class="submit_btn"><input type="hidden" name="id" value="<?php echo ($data["id"]); ?>"><input class="btn btn-primary le1 radius" type="submit" value="提交"></div>
            </form>
        </div>
    </div>
</body>

...
<body>
    ...
    <!-- bdright -->
    <div id="bdright">
        <div class="site"><a href="<?php echo U('Order/index');?>" class="active"><i class="icon-th-large"></i> 已发货订单</a></div>
        <div class="content">
            <div class="search">
                <i class="icon-search"></i>
                <form action="<?php echo U('Order/search2');?>" method="post" accept-charset="utf-8">
            ...
            <table class="table table-border table-bordered table-striped table-hover">
                ...
                <tbody>
                    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><a title="点击查看详细信息" href="<?php echo U('Order/info',array('id'=>$vo['mbid']));?>" target="_blank"><?php echo (r_mb_name($vo["mbid"])); ?></a></td>
                        ...
                        <td><?php if(empty($vo["kuaidi"])): ?><span class="red">未发货</span><?php else: echo ($vo["kuaidi"]); endif; ?></td>
                        <td><?php if(empty($vo["kuaidi_num"])): ?><span class="red">无</span><?php else: echo ($vo["kuaidi_num"]); endif; ?></td>
                        <td><a href="<?php echo U('Order/mod2',array('id'=>$vo['id']));?>" class="btn btn-success radius size-S">修改</a>　<a onclick="confirm('<?php echo U('Order/del',array('id'=>$vo['id']));?>','确定要删除吗？')" class="btn btn-danger radius size-S" value="删除">删除</a></td>
                    </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                    <tr>
                        <td colspan="20" class="text-r">
                            <form action="<?php echo U('Order/export',array('status'=>1));?>" accept-charset="utf-8" method="get">

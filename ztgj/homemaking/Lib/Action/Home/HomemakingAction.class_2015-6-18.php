...
class HomemakingAction extends Action {
    ...
    public function motherBabyQA(){
		...
		if(!session('bindid')){
	    	...
	    	foreach ($list as $key => $value) {
		    	...
		    	$homemakingCommunityReplyResult = $homemakingCommunityReply->field('homemaking_community_reply.fldContent,homemaking_user_bind.fldOpenId,homemaking_user_bind.fldOpenName')->where(array('fldCommunityId'=>$value['fldId']))->join('left join homemaking_user_bind on homemaking_community_reply.fldBindId = homemaking_user_bind.fldId')->limit(3)->select();
			...
			//$this->assign('communityLike',session('communityLike'));
    	...
    	if(!session('bindid')){
	    	...
	    	if(!in_array(I('param.id'),session('communityLike'))){
		    	...
		    	if($changeResult){
	    			...
	    			if($updateResult){
	    				//$this->ajaxReturn(session('communityLike'));
	    				$this->ajaxReturn('true');
    	...
    	if(IS_POST && !IS_AJAX){
    		...
    		if(!I('param.id')){
	    		...
	    		if (empty($homemakingCommunityContent)) {
		    		...
		    		//dump($homemakingCommunityContent);
		    		//dump($homemakingCommunityReplyResult);	
		    		//dump(json_encode($homemakingCommunityReplyResult));	
		    		//echo $homemakingCommunityReply->getDbError();
					if (IS_AJAX) {
    ...
    public function parentingInformationDetail(){
        ...
        $result=$content->field('fldId,fldTitle,fldCreateDate,fldContent')->where($data)->find();
    ...
    public function requirement(){
    	header("content-type:text/html;charset='UTF-8'");
    	//查询等级价格信息
    	if (session('gradePriceArray')) {
    		$gradePriceArray = session('gradePriceArray');
    	}else {
	    	...
	    	session('gradePriceArray',$gradePriceArray);
    	}
    	$gradePrice = D('HomemakingServicePrice');
    	$gradePriceResult = $gradePrice->relation(true)->select();
    	//dump($gradePriceResult[0]['homemaking_service_price_detail']);
    	
    	//获取等级证书映射
    	if (session('gradeCertificateMap')) {
    		$gradeCertificateMap = session('gradeCertificateMap');
    	}else {
	        ...
	        foreach ($homemakingServicePriceDetailResult as $key1 => $value1) {
	        	foreach ($value1['homemaking_certificate'] as $key2 => $value2) {
	        		if ($value2['fldStatus'] == 1) {
	        			$gradeCertificateMap[$value1['fldGrade']][] = $value2['fldName'];
	        		}
	        	}
	        }
	        session('gradeCertificateMap',$gradeCertificateMap);
    	}
    	...
    	/*if($this->_checkUrlcode(urldecode(I('param.type'))) && $this->_checkUrlcode(urldecode(I('param.type')))!='null'){
    		$type = ' fldPosition like '."'%".$this->_checkUrlcode(urldecode(I('param.type')))."%' ";  
    		$t = $this->_checkUrlcode(urldecode(I('param.type')));
    	}else{
    		$type = ' fldPosition like '."'%月嫂%' ";
    		$t = '月嫂';
    	}*/
    	if($this->_checkUrlcode(urldecode(I('param.type'))) && $this->_checkUrlcode(urldecode(I('param.type'))) =='育婴员'){
    		//$type = ' fldPosition like '."'%".$this->_checkUrlcode(urldecode(I('param.type')))."%' ";  
    		//$type = " fldPosition like '%育婴%' ";  
    		$type = " fldhometype = '04' and fldOnWork = 1 and fldgrade <> '' and fldgrade in ('高级','中级','初级') ";  
    		$t = '育婴员';
    	}else{
    		//$type = ' fldPosition like '."'%月嫂%' and fldSign = 1 ";
    		$type = " fldhometype = '03' and fldSign = 1 and fldgrade <> '' and fldgrade in ('A级','B级','C级','星级','特级','金牌') ";  
    		$t = '月嫂';
    	}
    	$this->assign('type',$t);
    	
    	//获取请求的心理价位
    	if(I('param.pay') && I('param.pay')!='null'){
	    	if($t == '月嫂'){
	    		$pay = I('param.pay','')=='null'?'':I('param.pay','');
	    		$grade = $gradePriceArray[1][$pay];
	    		$grade = " and fldGrade like "."'%".$grade."%' ";
	    		$this->assign('pay',$pay);
	    	}else if($t == '育婴员'){
	    		$pay = I('param.pay','')=='null'?'':I('param.pay','');
    	...
    	if(urldecode(I('param.place')) && urldecode(I('param.place'))!='null' && urldecode(I('param.place'))!='不限'){
    		$native = ' and fldNative like '."'%".urldecode(I('param.place'))."%' "; 
    	...
    	if(urldecode(I('param.locations')) && urldecode(I('param.locations'))!='null' && urldecode(I('param.locations'))!='不限'){
    		$location = ' and fldLocation like '."'%".urldecode(I('param.locations'))."%' "; 
    	...
    	if ($t == '月嫂') {
    		$condition = ' fldTakeBabyNum, ';
    	}else {
    		$condition = ' fldWorkYears, ';
    	}
    	
    	$page = I('param.page',1);  //获取请求的页数
    	$fldSign = ' fldSign = 0 ';
    	if (!session('workIdString')) {
	    	...
	    	session('workIdString',$workIdString);
    	}else {
    		$workIdString = session('workIdString');
    	}
    	...
    	//$result = $homeInfo->field(	'HomeInfo.fldID,fldName,fldage,fldNative,fldEducation,fldLanguage,fldType,fldPosition,fldMobile,fldPhoto,fldGuid,fldIDCard,fldSign,fldLocation,fldSpecialtyDish, '.$condition.' HomeInfoSign.fldGrade as fldGradeYuesao ,HomeInfo.fldGrade as fldGradeYuying')->join('LEFT JOIN HomeInfoSign ON HomeInfo.fldID = HomeInfoSign.fldhomeID')->where($type.$grade.$native.$location.' and HomeInfo.fldCompany=1 and HomeInfo.fldID not in ('.$workIdString.')')->limit(($page-1)*10,10)->select();
    	//$result = $homeInfo->field(	'fldID,fldName,fldage,fldNative,fldEducation,fldLanguage,fldType,fldPosition,fldMobile,fldPhoto,fldGuid,fldIDCard,fldSign,fldLocation,fldSpecialtyDish, '.$condition.' fldGrade')->where($type.$grade.$native.$location.' and fldCompany=1 and fldID not in ('.$workIdString.')')->limit(($page-1)*10,10)->select();
    	$result = $homeInfo->field(	'fldID,fldName,fldage,fldNative,fldEducation,fldLanguage,fldType,fldPosition,fldMobile,fldPhoto,fldGuid,fldIDCard,fldSign,fldLocation,fldSpecialtyDish, '.$condition.' fldGrade,fldhometype')->where($type.$grade.$native.$location." and fldCompany=1 and fldGrade <> '' and fldID not in (".$workIdString.")")->limit(($page-1)*10,10)->select();
    	
    	//dump($gradePriceArray);
    	...
    	foreach($result as $key=>$value){
	    	...
	    	$result[$key]['homemaking_certificate'] = $gradeCertificateMap[$value['fldGrade']];
	    	...
	    	//获取图片URL
	    	$userphoto = 'http://test.job5156.com/content/oto/house/img/2015-6/ed8eb89d9474884cbcfa72622d8b274d.jpg';
    	...
    	//返回等级价格信息
    	if($t == '月嫂'){
    	//if($t == '月嫂' && !empty($result)){
    		//$result['gradePrice'] = $gradePriceArray[1];
    		$this->assign('gradePriceArray',$gradePriceArray[1]);
    	}else if($t == '育婴员'){
    	//}else if($t == '育婴员' && !empty($result)){
    		//$result['gradePrice'] = $gradePriceArray[2];
    ...
    public function requirementDetail(){
    	...
    	/*
    	foreach ($wy as $k2 => $v2) {
    			if (($v1+($wy[$k2]-$wy[$k2-1]))>$v2) {
    				$s[$k1] = $workyearsMap[$v2];
    			}
    		}
    	*/
    	...
    	if (!(I('param.id')) || !I('param.type')) {
    		...
    		if (!IS_AJAX) {
	    		//查询个人信息
	    		if ($type == 1){
	    			$field = ',fldTakeBabyNum';
	    		}else{
	    			$field = ',fldWorkYears';
	    		}
		    	$homeInfo = D('HomeInfo');
		    	$result = $homeInfo->field('fldID,fldName,fldSex,fldMarryStatus,fldNative,fldPosition,fldType,fldLanguage,fldage,fldPhoto,fldSpecialty,fldSelfAppraise,fldSpecialtyDish,fldLocation,fldGrade,fldGuid'.$field)->where(array('fldID'=>$id))->find();
		    	//$result = $homeInfo->field(	'HomeInfo.fldID,fldName,fldage,fldNative,fldEducation,fldLanguage,fldType,fldPosition,fldMobile,fldPhoto,fldIDCard,fldSign,fldLocation,fldSpecialtyDish, '.$condition.' HomeInfoSign.fldGrade as fldGradeYuesao ,HomeInfo.fldGrade as fldGradeYuying')->join('LEFT JOIN HomeInfoSign ON HomeInfo.fldID = HomeInfoSign.fldhomeID')->where(array('HomeInfo.fldID'=>$id))->find();
		    	//echo $homeInfo->getDbError();
		    	...
		    	$result['homemaking_certificate'] = $gradeCertificateMap[$result['fldGrade']];
				...
				foreach ($homemakingServiceCommentResult as $key => $value) {
					...
					if(!empty($value['fldUploadFile'])){
						...
						if(!empty($homemakingImageUrlResult)){
							...
							/*foreach ($homemakingImageUrlResult as $k => $v) {
								if (!empty($v['fldFileName'])) {
									$homemakingImageUrlResult[$k] = 'http://ibm-store/school/weixin/'.$v['fldFileName'];
								}
							}*/
							//dump($homemakingImageUrlResultA);
					...
					/*if(!empty($value['fldUploadFile'])){
						$sysAttachFileResult = $sysAttachFile->field('fldFileName')->where(array('fldKey'=>$value['fldUploadFile']))->select();
						if(!empty($sysAttachFileResult)){
							$sysAttachFileResultA = array();
							$prefix = 't_';
							for ($i=0; $i < count($sysAttachFileResult); $i++) { 
								$s = strstr($sysAttachFileResult[$i]['fldFileName'],'.',TRUE);
								if(strlen($s) == 36){
									$sysAttachFileResultT = array();
									$sysAttachFileResultT[] = 'http://ibm-store/school/weixin/'.$sysAttachFileResult[$i]['fldFileName'];
									for ($j=0; $j < count($sysAttachFileResult); $j++) { 
										//if (strpos($sysAttachFileResult[$j]['fldFileName'],$s) !== false) {
										if (strstr($sysAttachFileResult[$j]['fldFileName'],$prefix.$s)) {
											$sysAttachFileResultT[] = 'http://ibm-store/school/weixin/'.$sysAttachFileResult[$j]['fldFileName'];
										}
									}
									$sysAttachFileResultA[] = $sysAttachFileResultT;
								}
							}
							//foreach ($sysAttachFileResult as $k => $v) {
							//	if (!empty($v['fldFileName'])) {
							//		$sysAttachFileResult[$k] = 'http://ibm-store/school/weixin/'.$v['fldFileName'];
							//	}
							//}
							$homemakingServiceCommentResult[$key]['fldUploadFile'] = $sysAttachFileResultA;
						}
					}*/
					...
					if (!empty($value['fldParentId'])) {
						...
						if (!empty($homeMycustomerResult)) {
	    					...
	    					if ($homeCustomerResult) {
		    					$homemakingServiceCommentResult[$key]['fldName'] = $homeCustomerResult['fldName'];
		    				}
						}
					}
				}
	    	}
	    	//dump($homemakingServiceCommentResult);
	    	//dump($homemakingServiceCommentResult[0]['fldUploadFile']);
	    	if (IS_AJAX) {
	    	...
	    	/*
	    	$homeMyCustServices = D('HomeMyCustServices','',DB_CONFIG3);
	    	$homeMyCustServicesResult = $homeMyCustServices->field('fldMycustomerID')->where(array('fldHomeID'=>$id))->select();
	    	//dump($homeMyCustServicesResult);
	    	//echo $homeMyCustServices->getLastSql();
	    	if($homeMyCustServicesResult){
		    	$homeMycustomer = D('HomeMycustomer');
		    	foreach ($homeMyCustServicesResult as $key => $value) {
		    		//dump($value);
		    		$homeMycustomerResult = $homeMycustomer->field('fldID,fldCustomerID,fldBeginDate,fldEndDate')->where(array('fldID'=>$value['fldMycustomerID']))->find();
		    		if ($homeMycustomerResult) {
		    			//dump($homeMycustomerResult);
		    			$homeMyCustServicesResult[$key]['fldBeginDate'] = $homeMycustomerResult['fldBeginDate'];
		    			$homeMyCustServicesResult[$key]['fldEndDate'] = $homeMycustomerResult['fldEndDate'];
		    			$homeCustomer = D('HomeCustomer');
	    				$homeCustomerResult = $homeCustomer->field('fldName')->where(array('fldID'=>$homeMycustomerResult['fldCustomerID']))->find();
	    				if ($homeCustomerResult) {
	    					$homeMyCustServicesResult[$key]['fldName'] = $homeCustomerResult['fldName'];
	    				}
	    				$homemakingServiceComment = M('homemaking_service_comment');
	    				$homemakingServiceCommentResult = $homemakingServiceComment->field('fldHomeID,fldAssessment,fldUploadFile')->where(array('fldParentId'=>$homeMycustomerResult['fldID']))->find();
	    				//echo $homemakingServiceComment->getLastSql();
	    				//dump($homemakingServiceCommentResult);
	    				if ($homemakingServiceCommentResult) {
	    					$homeMyCustServicesResult[$key]['fldAssessment'] = $homemakingServiceCommentResult['fldAssessment'];
	    					if(!empty($homemakingServiceCommentResult['fldUploadFile'])){
	    						$sysAttachFile = D('sysAttachFile');
	    						$sysAttachFileResult = $sysAttachFile->field('fldFileName')->where(array('fldKey'=>$homemakingServiceCommentResult['fldUploadFile']))->select();
	    						$homeMyCustServicesResult[$key]['fldUploadFile'] = $sysAttachFileResult;
	    					}
	    				}
		    		}
		    	}
	    	...
	    	dump($homeMyCustServicesResult);
	    	dump($homeMyCustServicesResult[0]['fldUploadFile']);
	    	*/
    ...
    public function requirementSubmit(){
    	header("content-type:text/html;charset='utf-8'");
    	/*session('uid',null);
    	session('openid','oeRcKtyS7fTNhXCYiEdkS517alv8');
    	session('requirement',I('post.'));
    	dump(session('requirement'));//die();
    	redirect(U('Account-login','',''));die();*/
    	if (session('uid')) {
    		$uid = session('uid');
    	}else {
	    	$account_info = D('AccountInfo');
    	...
    	if(IS_POST){
    		...
    		if(!$uid){//没有注册，则自动注册
                ...
                $redis->auth('job5156RedisMaster183');
			        ...
			        //注册成功推送绑定成功信息
		            if($uid){
    					$result = $this->sendWeixinTemplateMessage('changePassword','您已成功注册国民妈妈账户',$phone,$password,'','温馨提醒：请妥善保管您的账号信息，为防止密码泄露，建议您立即修改密码。点此修改用户名密码',$uid);
		            }
			        //die();
	            }else{
					$this->error('请在微信上打开，或者重新进入页面',U('Homemaking-requirementSubmit',array('v'=>time(),'type'=>I('param.type')),''));
            ...
            if($uid){
	    		...
	    		if(!$homeCustomerId){
		    		...
		    		$homeCustomerData['fldInfoFrom'] = '微信：国民妈妈';
		    		$homeCustomerData['lasteditby'] = 'admin';
		    		$homeCustomerData['lasteditdt'] = date('Y-m-d H:m:d',time());
		    		$homeCustomerData['fldCreateDate'] = date('Y-m-d H:m:d',time());
	    		...
	    		if($homeCustomerId){
		    		...
		    		$homemakingRequirementData['fldNative'] = I('post.native');
		    		$homemakingRequirementData['fldAge'] = I('post.age');
		    		//$homemakingRequirementData['fldDegree'] = I('post.education');
		    		$homemakingRequirementData['fldDegree'] = 0;
		    		$homemakingRequirementData['fldMarryStatus'] = I('post.marry');
		    		$homemakingRequirementData['fldLanguage'] = I('post.language');
		    		...
		    		$homemakingRequirementData['fldCreateDate'] = date('Y-m-d H:m:d',time());
		    		$homemakingRequirementData['lastEditby'] = 'admin';
		    		$homemakingRequirementData['lastEditdt'] = date('Y-m-d H:m:d',time());
		    		...
		    		if($homemakingRequirementId){
		    			//session('homemakingRequirementId',$homemakingRequirementId);
		    			$homeMycustomer = D('HomeMycustomer');
		    			...
		    			$homeMycustomerData['lasteditdt'] = date('Y-m-d H:m:d',time());
		    			$homeMycustomerData['lasteditby'] = 'admin';
		    			$homeMycustomerData['fldCreateDate'] = date('Y-m-d H:m:d',time());
	    		...
	    		if($addResult){
					 ...
					 //$this->wxpay($homemakingRequirementId,(($homeMycustomerData['fldserviceFee']+$homeMycustomerData['fldInsuranceFee'])*100),'预约订金','requirementSubmitSuccess');
					 if ($type == 1) {
					 	$this->wxpay($homemakingRequirementId,(($homeMycustomerData['fldserviceFee']+$homeMycustomerData['fldInsuranceFee'])*100),'预约订金','requirementSubmitSuccessYuesao');
					 }else {
					 	$this->wxpay($homemakingRequirementId,(($homeMycustomerData['fldserviceFee']+$homeMycustomerData['fldInsuranceFee'])*100),'预约订金','requirementSubmitSuccessYuyingsao');
					 }
    ...
    public function requirementSubmit4Login(){
    	...
    	if($uid && session('openid')){
    		...
    		if($homeCustomerId){
	    		...
	    		$homemakingRequirementData['fldNative'] = $post['native'];
	    		$homemakingRequirementData['fldAge'] = $post['age'];
	    		//$homemakingRequirementData['fldDegree'] = $post['education'];
	    		$homemakingRequirementData['fldDegree'] = 0;
	    		$homemakingRequirementData['fldMarryStatus'] = $post['marry'];
	    		$homemakingRequirementData['fldLanguage'] = $post['language'];
    ...
    public function customerCenter(){
    	...
    	/*	if(!session('openid')){
    		...
    		//$result = $this->sendWeixinNoReservationMessage();
    		//$result = $this->sendWeixinTemplateMessage('noReservation','您还没未预约服务需求!','客户中心','尚未预约','','电话预约请拨打：0769-87073668',0);
    		$result = $this->sendWeixinTemplateMessage('noReservation','您还没未预约服务需求!','您尚未预约月嫂/育婴嫂服务，暂不能查看客户中心。','请在菜单栏点击我要预约','','您也可以电话预约服务，免费预约热线0769-87073668',0);
    		import('ORG.Util.Jssdk');
			$jssdk = new JSSDK(APPID, APPSECRET);
			$signPackage = $jssdk->GetSignPackage();
			...
			/*
			//构造学历映射
			$degreeMap = array(0=>'不限',1=>'小学',2=>'初中',3=>'高中',4=>'中专',5=>'大专',6=>'本科及以上');
			//构造年龄映射
			$ageMap = array(0=>'不限',1=>'16-25岁',2=>'26-35岁',3=>'36-45岁',4=>'46-55岁');
			//构造语言映射
			$languageMap = array(0=>'不限',1=>'国语',2=>'粤语',3=>'粤语*国语');
			//构造婚育状况映射
			$marryMap = array(0=>'不限',1=>'未婚',2=>'已婚',3=>'已婚已育',4=>'离婚');
			*/
			$homemakingSetting = M('homemaking_setting');
			...
			//客户个人信息
	    	$homeCustomer = D('HomeCustomer');
	    	$homeCustomerInfo = $homeCustomer->field('fldID,fldName,fldSex,fldMobile,fldAddress,fldchildbirth,fldBadyBrithday,fldPerUserId,fldWeiXinCode')->where("fldPerUserId = ".$uid)->find();
	    	//没有客户信息，则提醒
	    	if(!$homeCustomerInfo){
	    		$result = $this->sendWeixinTemplateMessage('noReservation','您还没未预约服务需求!','您尚未预约月嫂/育婴嫂服务，暂不能查看客户中心。','请在菜单栏点击我要预约','','您也可以电话预约服务，免费预约热线0769-87073668',0);
	    		import('ORG.Util.Jssdk');
		    	...
		    	//$homeCustomerInfo['fldchildbirth'] = $homeCustomerInfo['fldchildbirth']->date;
		    	//echo $homeCustomer->getDbError();
		    	//dump($homeCustomerInfo['fldID']);
		    	//客户预约需求
		    	...
		    	foreach($homemakingRequirementInfo as $key=>$value){
		    		...
		    		if($homeMycustomerResult){
		    			...
		    			//$homeMyCustServicesResult = $homeMyCustServices->field('fldHomeID,fldFlag')->where('fldMycustomerID = 34 and (fldFlag = 1 or fldFlag = 2) ')->find();
		    		...
		    		if(homeMyCustServicesResult){
		    			...
		    			if($homeInfoResult){
		    				...
		    				$homeInfoResult['Marry'] = $marryMap[$homeInfoResult['fldMarryStatus']];
		    				...
		    				if (!empty($homeInfoResult['fldGrade'])) {
		    					$homeInfoResult['homemaking_certificate'] = $gradeCertificateMap[$homeInfoResult['fldGrade']];
		    				}
	    	...
	    	//dump($homemakingRequirementInfo);
	    	$time = session('time');
	        $newtime = time();
	        $losetime = $newtime - $time;
	        $losetime = 120 - $losetime;
			...
			if ($homemakingServiceCommentResult) {
				...
				/*if (!empty($homemakingServiceCommentResult['fldUploadFile'])) {
					...
					if(!empty($sysAttachFileResult)){
						...
						$homemakingServiceCommentResult['fldUploadFile'] = $sysAttachFileResultA;
					}
				}*/
				
				//dump($sysAttachFileResultA);
				//计算评分
				if (I('param.t') == 1) {
    ...
    public function unpay(){
		if (I('param.id') && session('uid')) {
			...
			if ($homeCustomerResult['fldID'] == $homeMycustomerResult['fldCustomerID']) {
				//session('homemakingRequirementId',$homeCustomerResult['fldID']);
				$homemakingRequirement = M('homemaking_requirement');
				$type = $homemakingRequirement->field('fldType')->where(array('fldId'=>I('param.id')))->find();
				if ($type['fldType'] == 1) {
				 	$this->wxpay(I('param.id'),(($homeMycustomerResult['fldserviceFee']+$homeMycustomerResult['fldInsuranceFee'])*100),'预约订金','unpayYuesao');
				}else {
				 	$this->wxpay(I('param.id'),(($homeMycustomerResult['fldserviceFee']+$homeMycustomerResult['fldInsuranceFee'])*100),'预约订金','unpayYuyingsao');
				}
			}
		}else {
			$this->error('非法操作');
    ...
    public function upload(){
		...
		if(!$upload->upload()) {// 上传错误提示错误信息
			$this->_wlog(date('Y-m-d H:i:m',time()).':'.$upload->getErrorMsg()."\r\n\r\n");
			...
			if ($timage && $image) {
				...
				$this->_wlog(date('Y-m-d H:i:m',time()).':'.$prefix.$info[0]['savename']."\r\n");
				$this->_wlog(date('Y-m-d H:i:m',time()).':'.$timage."\r\n");
				$this->_wlog(date('Y-m-d H:i:m',time()).':'.$info[0]['savename']."\r\n");
				$this->_wlog(date('Y-m-d H:i:m',time()).':'.$image."\r\n");
				$this->_wlog(date('Y-m-d H:i:m',time()).':'.json_encode(array('id'=>$info[0]['savename'],'src'=>$timage,'originsrc'=>$image))."\r\n\r\n");
				//dump(session(I('param.cid').'uploadFileName'));
    ...
    /**
	 ...
	 public function delete(){
	 	 if (I('post.id')) {
			 $prefix = 't_';
	 	 	 //$sysAttachFile = D('sysAttachFile');
		 	 //$result = $sysAttachFile->where(array('fldFileName'=>I('post.id')))->delete();
		 	 //$result = $result && $sysAttachFile->where(array('fldFileName'=>'t_'.I('post.id')))->delete();
	 	 	if(!session(I('param.cid').'uploadFileName')){
				die('false');
			}else{
				$uploadFileName = session(I('param.cid').'uploadFileName');
    ...
    public function crossDomainUpload($fileLocation = '/data/www/html/newer/homemaking/Uploads/Weixin/userphoto.jpg'){
    	//$uri = "http://192.168.8.198/open/api/common/upload";
    	$uri = "http://test.job5156.com/open/api/common/upload";
		// post参数数组
		$data = array (
		        ...
		        //'file' => '@'.getcwd().'\Uploads\Weixin\userphoto.jpg',
		        //'file' => '@/data/www/html/newer/homemaking/Uploads/t_73818581-3E73-9696-D397-1F323DB37B7E.gif',
		        'file' => '@'.$fileLocation,
		        'type' => 'img',
		        'proType' => 'house',
		...
		//释放
		curl_close ( $ch );
		//$current=preg_match('test',$current,$result);
    ...
    public function upload(){
		...
		if(!$upload->upload()) {// 上传错误提示错误信息
			...
			$info =  $upload->getUploadFileInfo();
			$conn = ftp_connect("192.168.0.19") or die("Could not connect");
			ftp_login($conn,"jz","jz0769");
			//利用ftp创建目录
			...
			$prefix = 't_';
			//开始上传
			$result = ftp_put($conn,$prefix.$info[0]['savename'],getcwd().$upload->savePath.$prefix.$info[0]['savename'],FTP_BINARY) && ftp_put($conn,$info[0]['savename'],getcwd().$upload->savePath.$info[0]['savename'],FTP_BINARY);
			if($result){
				if(!session('uploadFileName')){
					session('uploadFileName',array($info[0]['savename']=>$info[0]['name'],$prefix.$info[0]['savename']=>$prefix.$info[0]['name']));
				}else{
					$uploadFileName = session('uploadFileName');
					$uploadFileName[$info[0]['savename']] = $info[0]['name'];
					$uploadFileName[$prefix.$info[0]['savename']] = $prefix.$info[0]['name'];
					session('uploadFileName',$uploadFileName);
				}
				unlink(getcwd().$upload->savePath.$prefix.$info[0]['savename']);
				unlink(getcwd().$upload->savePath.$info[0]['savename']);
				echo json_encode(array('id'=>$info[0]['savename'],'src'=>'http://ibm-store/school/weixin/'.$prefix.$info[0]['savename'],'originsrc'=>'http://ibm-store/school/weixin/'.$info[0]['savename'],I('post.')));
			}else{
				echo 'false';
			}
			ftp_close($conn);
	 ...
	 public function delete(){
	 	 if (I('post.id')) {
		 	...
		 	if($result){
			 	$conn = ftp_connect("192.168.0.19") or die("Could not connect");
				ftp_login($conn,"jz","jz0769");
				echo ftp_delete($conn,I('post.id')) && ftp_delete($conn,$prefix.I('post.id'));
				ftp_close($conn);
			}
	 	 }else {
	 	 	echo 'false';
	 ...
	 public function uploadFtp(){
		$conn = ftp_connect('192.168.0.19') or die ("error");
		$result = ftp_login($conn,'jz','jz0769');
		dump($result);
		
		//ftp_chdir($conn, '/'); 
		//if(ftp_put($conn,'6968DD37-D673-42CE-A8B3-5A19E74256FE.jpg','./Uploads/4307DA98-BF3D-4D97-9C6E-797A20CDA8A1.png', FTP_ASCII)){
		if(ftp_put($conn,'userphoto.jpg','E:/CYH/work/jiazheng/SVN/homemaking/Uploads/userphoto.jpg', FTP_BINARY)){
		  echo "上传成功";
		} else {
		  echo "上传失败";
		}
    ...
    /**
	  ...
	  * @author RohoChan<[email]rohochan@gmail.com[/email]>
	  **/
    public function sendWeixinTemplateMessage($type,$first='国民妈妈',$keyword1='',$keyword2='',$keyword3='',$remark='广东智通人才连锁股份有限公司',$uid=0){
        ...
        if($type == 'changePassword'){
	        ...
	        $message['url'] = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=".APPID."&redirect_uri=".DOMAIN."Homemaking-changePassword-id-".$uid."-u-".$keyword1."-p-".$keyword2."&scope=snsapi_base&state=123#wechat_redirect";
        	...
        	//$message['template_id'] = "8fs3AgFyRb7p76a9jl-dHT5wFSqfl5fYiD2f1Ly0JMo";
        	...
        	//$message['template_id'] = "nH99LMYs60NBJz1R_43KzJ7I-ZVw3ulFrQTR-5C9hfs";
    ...
    public function trainSubmit(){
    	...
    	if(IS_POST){
    		...
    		$homeTrainOrderData['lasteditdt'] = date('Y-m-d H:m:d',time());
    		$homeTrainOrderData['fldcreateDate'] = date('Y-m-d H:m:d',time());
    ...
    public function competeSitter(){
    	...
    	if(IS_POST){
    		...
    		if(!$homeInfoId){
	    		...
	    		$homeInfoData['lasteditdt'] = date('Y-m-d H:m:d',time());
	    		$homeInfoData['fldCreater'] = 'admin';
	    		$homeInfoData['fldCreateDate'] = date('Y-m-d H:m:d',time());
    		...
    		if($homeInfoId){
	    		...
	    		if(!$homemakingSitterId){
	    			...
	    			$homemakingSitterData['fldCreateDate'] = date('Y-m-d H:m:d',time());
			    	$homemakingSitterData['lastEditby'] = 'admin';
			    	$homemakingSitterData['lastEditdt'] = date('Y-m-d H:m:d',time());
	    		...
	    		foreach(I('post.certificate1') as $key=>$value){
	    			...
	    			if(!$homemakingPersonCertificateId){
			    		...
			    		$homemakingPersonCertificateData[$key]['lastEditdt'] = date('Y-m-d H:m:d',time());
			    		$homemakingPersonCertificateData[$key]['fldOwner'] = 'admin';
			    		$homemakingPersonCertificateData[$key]['fldCreateDate'] = date('Y-m-d H:m:d',time());
